...
import {
  Image, Upload, Loader, AlertTriangle, CheckCircle, Shield, Trash2, X, Plus,
  Globe, Search, Camera, ExternalLink, Link2, Info, Bot, Eye,
...
const ImageVerification = () => {
  ...
  const handleFilesSelect = (e) => {
    ...
    if (validFiles.length < selected.length) toast.error('Some files were skipped (invalid type or >15MB)')
  ...
  const handleVerifyUpload = async () => {
    ...
    try {
      ...
      const res = await renterAPI.analyzeUploadedImages(formData)
      setResult(res.data)
      toast.success('Image verification complete!')
    } catch (error) {
      console.error('Image verification failed:', error)
      toast.error(error.response?.data?.detail || 'Image verification failed. Please try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleVerifyUrl = async () => {
    if (!imageUrl.trim()) return toast.error('Please enter a listing URL')
    setLoading(true)
    setResult(null)
    try {
      const res = await renterAPI.analyzeImages([imageUrl])
      setResult(res.data)
      toast.success('Image crawl & verification complete!')
    } catch (error) {
      console.error('Image URL verification failed:', error)
      toast.error(error.response?.data?.detail || 'Image crawl & verification failed. Please try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <TenantLayout title="Verify Property Images" subtitle="Detect fake, stolen, or AI-generated listing photos">
      <div className="max-w-4xl mx-auto space-y-6">
        {/* Info Banner */}
        <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-200 dark:border-emerald-800">
          <div className="flex items-start gap-3">
            <Shield className="h-5 w-5 text-emerald-600 mt-0.5 shrink-0" />
            <div>
              <h4 className="text-sm font-semibold text-surface-900 dark:text-white">How Image Verification Works</h4>
              <p className="text-xs text-surface-600 dark:text-surface-400 mt-1">
                Our AI analyzes images using <strong>CNN scene classification</strong>, <strong>forensic AI-generation detection</strong> (noise residuals, frequency spectrum, patch statistics), and <strong>Google Cloud Vision reverse image search</strong> to find if photos appear elsewhere on the web — a common sign of stock images or rental fraud.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex bg-surface-100 dark:bg-surface-800 rounded-xl p-1 mb-6">
            <button onClick={() => { setMode('upload'); setResult(null) }}
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${
                mode === 'upload' ? 'bg-white dark:bg-surface-700 text-surface-900 dark:text-white shadow-sm' : 'text-surface-500 hover:text-surface-700'
              }`}>
              <Upload className="h-4 w-4" /> Upload Images
            </button>
            <button onClick={() => { setMode('url'); setResult(null) }}
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${
                mode === 'url' ? 'bg-white dark:bg-surface-700 text-surface-900 dark:text-white shadow-sm' : 'text-surface-500 hover:text-surface-700'
              }`}>
              <Globe className="h-4 w-4" /> Crawl from Listing URL
            </button>
          </div>
          ...
          {mode === 'upload' ? (
            <>
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <p className="text-sm text-surface-500 dark:text-surface-400">Upload photos from a listing to check if they are real</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-4">
                ...
                <button onClick={() => fileInputRef.current?.click()}
                  className="aspect-square rounded-xl border-2 border-dashed border-surface-300 dark:border-surface-600 flex flex-col items-center justify-center gap-2 hover:border-tenant-500 transition-colors cursor-pointer">
              ...
              <div className="flex gap-3">
                <button onClick={handleVerifyUpload} disabled={loading || files.length === 0} className="btn btn-lg btn-primary flex-1">
                  {loading ? <><Loader className="h-5 w-5 animate-spin" /> Searching the web for matches...</> : <><Search className="h-5 w-5" /> Verify {files.length} Image{files.length !== 1 ? 's' : ''}</>}
                </button>
                {files.length > 0 && (
            ...
            <>
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h3 className="text-lg font-display font-bold text-surface-900 dark:text-white">Crawl Listing URL</h3>
                  <p className="text-sm text-surface-500 dark:text-surface-400">Paste a listing URL — we will extract all images and verify each one</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <Link2 className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-surface-400" />
                    <input type="url" value={imageUrl} onChange={e => setImageUrl(e.target.value)}
                      className="input-field pl-10" placeholder="https://www.kijiji.ca/v-apartments/..." />
                  </div>
                  <p className="text-xs text-surface-500 mt-1.5">Supports Kijiji, Craigslist, Facebook Marketplace, Rentals.ca, RentFaster, and more</p>
                </div>
                <button onClick={handleVerifyUrl} disabled={loading || !imageUrl.trim()} className="btn btn-lg btn-primary w-full">
                  {loading ? <><Loader className="h-5 w-5 animate-spin" /> Crawling &amp; analyzing images...</> : <><Search className="h-5 w-5" /> Crawl &amp; Verify Images</>}
                </button>
              </div>
            </>
          )}
        ...
        {result && (
          <div className="card p-6 animate-fade-in-up space-y-5">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-display font-bold text-surface-900 dark:text-white">Verification Results</h3>
              {result.image_count > 0 && <span className="text-sm text-surface-500">{result.image_count} image{result.image_count !== 1 ? 's' : ''} analyzed</span>}
            </div>

            {/* Overall risk */}
            {result.overall_risk_level && (() => {
              const isGood = result.overall_risk_level === 'authentic' || result.overall_risk_level === 'likely_authentic'
              const isBad = result.overall_risk_level === 'suspicious' || result.overall_risk_level === 'likely_fake'
              return (
                <div className={`flex items-center justify-between p-4 rounded-xl ${
                  isBad ? 'bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-800' :
                  !isGood ? 'bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-800' :
                  'bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-800'
                }`}>
                  <div className="flex items-center gap-3">
                    {isBad ? <AlertTriangle className="h-5 w-5 text-red-600" /> :
                     !isGood ? <Info className="h-5 w-5 text-amber-600" /> :
                     <CheckCircle className="h-5 w-5 text-green-600" />}
                    <div>
                      <p className="text-sm font-semibold text-surface-900 dark:text-white">Overall Assessment</p>
                      <p className="text-xs text-surface-500">
                        {result.suspicious_images_count > 0 ? `${result.suspicious_images_count} suspicious image${result.suspicious_images_count !== 1 ? 's' : ''} found` : 'All images appear legitimate'}
                      </p>
                    </div>
                  </div>
            ...
            {/* Summary */}
            {result.summary && (
              <div className="p-4 rounded-xl bg-surface-50 dark:bg-surface-800 text-sm text-surface-700 dark:text-surface-300 leading-relaxed">
                {result.summary}
              </div>
            )}
            ...
            {result.images?.length > 0 && (
              <div className="space-y-3">
                ...
                {result.images.map((img, i) => {
                  const badge = getRiskBadge(img.risk_level)
                  const isRisky = img.risk_level === 'suspicious' || img.risk_level === 'likely_fake' || img.risk_level === 'uncertain'
                  return (
                    <div key={i} className={`p-4 rounded-xl border ${
                      isRisky ? 'bg-red-50/50 dark:bg-red-500/5 border-red-200 dark:border-red-800' : 'bg-surface-50 dark:bg-surface-800 border-surface-200 dark:border-surface-700'
                    }`}>
